...
func HasThumb(blogId, userId int64, redisClient *redis.Client) (bool, error) {
	...
	key := fmt.Sprintf("%s%d", USER_THUMB_KEY_PREFIX, userId) //一个用户对应多个博客
...
func SavaThumb(blogId, userId int64, redisClient *redis.Client) error {
	set := redisClient.HSet(context.Background(), fmt.Sprintf("%s%d", USER_THUMB_KEY_PREFIX, userId), strconv.FormatInt(blogId, 10), 1)
	return set.Err()
}

func DeleteThumb(blogId int64, userId int64, client *redis.Client) error {
	return client.HDel(context.Background(), fmt.Sprintf("%s%d", USER_THUMB_KEY_PREFIX, userId), strconv.FormatInt(blogId, 10)).Err()
}

func MayHaveThumb(userId, blogId int64, redisClient *redis.Client) bool {
	key := fmt.Sprintf("%s%d", USER_THUMB_BLOOM_KEY, userId)
